...
- Initialization: useState(0) initializes count to 0 and provides a setCount function to update it.
- State Update: The increment function updates the state by calling setCount(count + 1).
  Rendering State: The current state (count) is rendered inside the <p> tag.
